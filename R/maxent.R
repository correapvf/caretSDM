...
#' 
#' This function will use the java implementation of MaxEnt by Phillips, Dudik and Schapire, instead of the package maxnet.
#' Check the MaxEnt documentation for more information.
#' Only suitable for presence/absence or presence/background data.
#' This is similar to \code{\link[dismo]{maxent}}, with less options (for instance, this will not accept rasters), but it is faster, specially the predict function.
...
#' @param reg Regularization features to use. l - linear, q - quadratic, p - product, h - hinge
#' @param beta Beta multiplier
#' @param niter Maximum number of iterations
#' @param outputType One of 'Cloglog', 'Logistic', 'Cumulative', 'Raw'
#' @param thrtype Which threshold type to use. One of 'Min_Presence','10\%_Presence','Sens=Spec','MaxSens+Spec','Balance','Entropy'
...
maxent.default <- function(x, y, reg = "lqph", beta = 1, niter = 500, outputType = "Cloglog",
                   ...
                   filesPath = paste0(tempdir(), '/maxent'), maxentPath = NULL, ...) {
    ...
    args <- c(paste0('betamultiplier=', beta),
    ...
    spp.xy <- 1:nrow(spp.val)
    model_pres <- paste0(outdir, "/presence")
    
    data.table::fwrite(data.table::data.table(species, x=spp.xy, y=spp.xy, spp.val), file=model_pres, quote=FALSE)
    ...
    # make model
    invisible(system2('java', args = c("-mx1024m", "-jar", maxentPath, "-e", model_bg, "-s", model_pres, "-o",
                                       outdir, "-m", niter, "-z", "-a", "plots=false", "pictures=false", paste0("outputformat=",tolower(outputType)), 
                                       "removeduplicates=false", "addsamplestobackground=false", "maximumbackground=100000", args)))
...
#' @param na.action A function to specify the action to be taken if NAs are found. The default action is for the procedure to fail. 
#' @rdname maxent
#' @export
