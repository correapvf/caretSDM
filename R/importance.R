...
#' @param method A method to calculate correlations, passed to \code{\link[stats]{cor}}.
...
#' @rdname varImp2
#' @export
varImp2.train <- function(model, nperm = 1, errorFunction = ci_95, method = "pearson", ...) {
...
#' @rdname varImp2
#' @export
plot.varImp2 <- function(x, scale = TRUE, plot_errorbar = TRUE, ...) {
    tmp <- summary(x, scale)
...
#' @rdname varImp2
#' @export
summary.varImp2 <- function(object, scale = TRUE, ...) {
...
print.varImp2 <- function(x, scale = TRUE, ...) {
    ...
    tmp = summary(x, scale)
    print(dcast(tmp, method + variable ~ data, value.var = "importance"), nrows=20)
}
