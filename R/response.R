...
globalVariables(c("factors", "predictors", "s"))
...
#' @rdname response
#' @export
response.train <- function(model, fixedvarFunction = mean, errorFunction = ci_95, n = 100, progress = TRUE, ...) {
    ...
    if (!is.null(errorFunction)) {
        ...
        results <- foreach(i=model$control$index, s=model$control$seeds) %op% {
            ...
            response.helper(tmp.model, fixedvarFunction, n, response_numeric, coefs, model.type)
        }
        ...
        # only use the the final model
        out <- response.helper(model, fixedvarFunction, n, response_numeric, coefs, model.type)
...
plot.response.train <- function(x, plot_errorbar = TRUE, plot_rugs = TRUE, plot_thr = TRUE, ...) {
    ...
    lim <- if (x$modelType == "Classification") c(0, 1) else range(x$num$response)
...
response.helper <- function(model, fixedvarFunction, n, response_numeric, coefs, model.type) {
    
    # create a few variables to use
    
    newdata <- model$trainingData[, coefs]
    varfactor <- names(model$xlevels)
    varnumeric <- colnames(newdata)[!(colnames(newdata) %in% varfactor)]
    ...
    # for numeric
    if (length(varnumeric) > 0) {
        fixedData2 <- fixedData[rep(1, (n * length(varnumeric))),]
        variable <- rep(varnumeric, each=n)
        
        # replace values in fixedData for a sequence for each variable
        for (var in varnumeric) {
            index <- which(variable==var)
            fixedData2[index, var] <- response_numeric[index] 
        }
        
        # make predictions
        if (length(varfactor) > 0) 
            fixedData2[, varfactor] <- mapply(x=fixedData2[, varfactor], levels=model$xlevels, factor, SIMPLIFY = FALSE)
        
        pred <- predict2(model, fixedData2, type = model.type)
        
        out$num <- data.table(variable = factor(variable, levels = varnumeric), 
    ...
    if (length(varfactor) > 0) {
        ...
        fixedData2 <- fixedData[rep(1, sum(tmp2_l)),]
        variable <- rep(varfactor, tmp2_l)
        tmp2_u <- unlist(tmp2_u)
        
        for (var in varfactor) {
            index <- variable==var
            fixedData2[index, var] <- as.character(tmp2_u[index])
        }
        ...
        out$fact <- data.table(variable = factor(variable, levels = varfactor),
                               factors = tmp2_u,
                               response = pred)
    } else {
