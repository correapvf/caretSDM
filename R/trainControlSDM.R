...
#' 
#' This function has two more keys arguments compared to \code{caret::trainControl}: 'spatial.folds' and 'presence.only'.
...
#' When presence.only is set to \code{TRUE}, than the sampling is done only in the presence data, and all absence records are used in each replicate.
...
#' @param y Response variable. Only used if \code{presence.only} is \code{TRUE}. First level is assumed to be "presence" data.
...
trainControlSDM <- function(method = "boot", number = 10, repeats = NA, p = 0.75, y = NULL, spatial.folds = NULL, 
    ...
    if (is.null(spatial.folds)) {
        ...
        if (presence.only) {
            ...
            control <- trainControl(method = method, number = number, repeats = repeats, p = p, index = index,
                                    indexOut = indexOut, classProbs = classProbs, summaryFunction = summaryFunction, ...)
        ...
        control <- trainControl(method = method, number = number, repeats = repeats, p = p, index = indexFold,
                                indexOut = indexoutFold, classProbs = classProbs, summaryFunction = summaryFunction, ...)
