...
globalVariables(c("prob_threshold" ,"Kappa", "Detection", "Dist", "Detection Prevalence", "parameters"))
...
#' @param thr.interval A value to create probability thresholds cutoffs intervals to be evaluated. Should be between 0 and 0.1.
#' @param final logical. Should only the final tuning parameters chosen by train be used?
#' @param add.statistics A character vector indicating additional statistics to calculate. See \code{\link[caret]{thresholder}} for a list of possible values.
#' Note that 'Sensitivity', 'Specificity', 'Kappa', 'Dist' and 'Detection Prevalence' are always calculated.
#' @param obs.prev Observed prevalence, in case your data is taken from a larger dataset. Defaults to observed prevalence from training data.
...
#' @return For the method \code{plot}, if \code{plot = TRUE}, a ggplot is returned. If \code{plot = FALSE}, a list of data.tables
...
#' @rdname thresholder2
#' @export
summary.thresholder2 <- function(object, which.statistics = "J", maximize=ifelse(which.statistics == "Dist", FALSE, TRUE), ...) {
    if (maximize) {
